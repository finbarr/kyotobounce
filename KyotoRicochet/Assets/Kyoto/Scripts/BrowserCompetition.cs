...
namespace Kyoto
{
    ...
    {public Vector3 center;public float radius;public string surface;}
    [Serializable] public class BrowserChallenge
    {public string id,name,layout,physics,requiredSurface,throwModel;public int revision;public BrowserDisk start,goal;}
    public sealed partial class BrowserSession
    {
        public const string SimulationVersion="kyoto-p3-1";
        ...
        [Serializable] class ThrowResult
        {
            ...
            public bool success;public int score,surfaces,impacts;public float duration;
        ...
        bool CompetitionCommand(Command c)
        {
            if(c.type=="place")
            {
                ...
                if(Physics.Raycast(c.origin,c.direction.normalized,out var hit,250,CollisionLayers.StaticBallMask,QueryTriggerInteraction.Ignore)
                   &&hit.normal.y>.995f)
                {
                    var disk=new BrowserDisk{center=hit.point,radius=c.radius,surface=SurfaceId(hit.collider)};
                    reply.ok=ValidateDisk(disk,c.slot=="start",out reply.message);reply.disk=disk;
                }
                else reply.message="Choose a fixed horizontal floor or landing.";
                Send(reply);return true;
            }
            if(c.type=="validate")
            {
                bool ok=ValidateDisk(c.start,true,out string message)&&ValidateDisk(c.goal,false,out message);
            ...
            if(c.type=="select")
            {
                ...
                if(ok&&c.challenge!=null)
                {
                    ...
                    if(ok)ok=ValidateDisk(c.challenge.start,true,out message)&&ValidateDisk(c.challenge.goal,false,out message);
                }
                if(ok)
                {
        ...
        void BeginRecording()
        {
            distinct.Clear();impactTimes.Clear();impactCount=0;goalDwell=sleepDwell=0;replayPoses.Clear();replayContacts.Clear();
            scorePoseCursor=0;
        ...
        void Finish(bool success,string reason)
        {
            ...
            Send(new ThrowResult{attempt=attempt,id=owner.state.id,reason=reason,layout=state.layout,profile=state.profile,challenge=activeChallenge,
                success=success,score=success?1000+100*distinct.Count:0,surfaces=distinct.Count,impacts=impactCount,duration=ball.Clock,
