...
const page=await context.newPage();page.setDefaultTimeout(120000);const shots=[],commands=[],errors=[];
page.on('pageerror',e=>errors.push(String(e)));page.on('websocket',ws=>ws.on('framesent',f=>{try{const m=JSON.parse(String(f.payload));if(m.type!=='input')commands.push({at:Date.now(),...m});}catch{}}));
let status='fail';
try{
...
}finally{await writeFile(resolve(out,'result.json'),JSON.stringify({status,scope:'Actual full-game DOM/keyboard completion, at most one retry; final layout binding must match native proof',shots,commands,errors},null,2));await context.close();await browser.close();}
