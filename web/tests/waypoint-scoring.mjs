...
const c={id:'waypoint-test',revision:1,name:'Targets',creator:'test',layout:'test',physics:'kyoto-p3-3',throwModel:'robot-v4',scoring:'waypoint-v2',start,goal:null,waypoints:targets};
...
const earned=scoreAttempt(result());assert.equal(earned.waypointBase,70000);assert.equal(earned.total,70000);assert.equal(earned.waypointMultiplier,8,'Label means NEXT waypoint award multiplier');
assert.equal(scoreAttempt(result({challenge:{...c,goal}})).total,earned.total,'Missing a destination never removes waypoint points');
const dest=scoreAttempt(result({challenge:{...c,goal},success:true,destinationReached:true}));assert.equal(dest.destinationBonus,earned.total);assert.equal(dest.total,140000);
assert.equal(scoreAttempt(result({challenge:{...c,goal,waypoints:[]},waypointHits:[],destinationReached:true})).total,10000,'Destination-only course earns the base bonus');
...
const store=new Store(':memory:');try{
 ...
 worker.capabilities=['waypoint-v2'];
 const save={type:'save-challenge',name:'Targets',start,goal:null,waypoints:targets,scoring:'waypoint-v2'};
