...
try{
 ...
 await returning.request('select-challenge',{challengeId:null},'selected');returning.close();
 const exploring=await enter(token);assert.equal(exploring.messages.findLast(m=>m.type==='session').challenge,null,'An explicit free-exploration choice survives reconnecting');
 console.log('PASS native first-level spawn, saved stage restoration and explicit free-exploration restoration');
