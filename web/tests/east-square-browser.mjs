...
const courses=[];
for(const v of views){await delay(1050);const start=await c.place(v,.5,'start');courses.push((await c.request('save-challenge',{name:`K027 ${stage} ${v.name}`,start,goal:await c.place({x:106,y:34.62,z:-12},.6,'goal'),waypoints:[],scoring:'waypoint-v3'},'saved-challenge')).challenge);}
