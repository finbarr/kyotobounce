...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  for(const yaw of [-150,0,45,90,180])for(const phase of ['Flight','Result']){
   const a=createAvatar(asset),parent=new T.Group();parent.rotation.y=.3;parent.add(a.group);a.group.position.set(2,.1,4);a.group.rotation.y=Math.PI-yaw*Math.PI/180;
   ...
   rows.push({yaw,phase,error:forward.angleTo(direction),drift:before.angleTo(a.head.quaternion)});
   ...
   poseAvatar(a,p,phase,state,-1);const seek=a.head.quaternion.clone();const fresh=createAvatar(asset);parent.add(fresh.group);fresh.group.position.copy(a.group.position);fresh.group.quaternion.copy(a.group.quaternion);poseAvatar(fresh,p,phase,state,-1);
