...
try{
 ...
 await page.evaluate(async()=>{
  ...
  window.score=n=>({version:'waypoint-v3',waypointCount:n,waypointIds:Array.from({length:n},(_,i)=>`target-${i+1}`),waypointHits:Array.from({length:n},(_,i)=>({waypointId:`target-${i+1}`,time:i,point:{x:i,y:1,z:0},normal:{x:0,y:1,z:0},surface:'floor'})),waypointBase:(n?10000*2**n:0),waypointMultiplier:2**n,destinationReached:false,destinationBonus:0,bankMultiplier:1,bankBonus:0,comboMultiplier:2**n,total:(n?10000*2**n:0),potential:(n?10000*2**n:0),styleBanks:0,landingMultiplier:0});
 ...
 for(let n=1;n<=5;n++){
  ...
  shown.push(await page.locator('#combo-mult').textContent());assert.equal(shown.at(-1),({2:'SPARK',4:'GOLD RUSH'})[n]||`×${2**n}`);
 ...
 assert.equal(await page.locator('#combo-spectacle').isVisible(),false);assert.equal(await page.locator('#combo-total').textContent(),'40,000');assert.equal(await page.evaluate(()=>calls.filter(c=>['waypoint','special'].includes(c[0])).length),5);
 ...
 await page.evaluate(()=>feedback.result({attempt:'fixture-shot',breakdown:{...score(6),outcome:'miss'}}));assert.equal(await page.locator('#combo-mult').textContent(),'640,000');assert.match(await page.locator('#combo-sub').textContent(),/POINTS BANKED/);assert.ok(await page.evaluate(()=>{frequencies.length=0;sound.cue('result','miss');return frequencies.includes(440);}), 'Earned waypoint miss uses positive result phrase');await page.screenshot({path:`${out}/destination-missed-points-kept.png`});
 ...
 await writeFile(`${out}/result.json`,JSON.stringify({status:'pass',scope:'Isolated browser fixtures; compatible native waypoint worker not supplied',shown,calls:await page.evaluate(()=>calls)},null,2));console.log('PASS earned x2/x4/x8/x16/x32, duplicate IDs, rewind, retained score, destination bonus and reduced motion');
