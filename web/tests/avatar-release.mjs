...
try{
 ...
 const result=await page.evaluate(async()=>{
  ...
  for(const character of ['ori','koma','don'])for(const yaw of [-150,0,45,90])for(const [top,kick]of [[0,0],[200,0],[-200,0],[0,200],[0,-200],[140,140],[-140,-140]]){
   ...
   results.push({character,yaw,top,kick,error:endpoint.distanceTo(target),upperLength:upper.distanceTo(elbow),forearmLength:elbow.distanceTo(wrist)});
  }
 ...
 await writeFile(`${out}/result.json`,JSON.stringify({status:'pass',scope:'Programmatic endpoint and pose stability using the actual glTF rig; not ordinary-input or full anatomical acceptance',...result},null,2));console.log('PASS 84 rig release endpoints across three characters, fixed limb lengths, no accumulated pose rotation');
