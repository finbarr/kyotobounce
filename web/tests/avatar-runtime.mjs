...
const context=await browser.newContext({viewport:{width:1280,height:900},deviceScaleFactor:.2,recordVideo:{dir:out,size:{width:1280,height:900}}});const page=await context.newPage(),errors=[],evidence=[],impacts=[];
...
async function capture(name){const s=await state();evidence.push({name,state:s});// Video records ordinary input continuously; avoid blocking software GPU readback here.
...
try{
 ...
 await writeFile(`${out}/result.json`,JSON.stringify({status:'pass',scope:'Native browser input: idle, forward, stop, strafe, backward, orbit, fast, throw, bounces, recall',evidence,impacts,errors},null,2));
