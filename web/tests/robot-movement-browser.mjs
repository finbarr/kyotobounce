...
const deadline=setTimeout(()=>{errors.push('180 second review budget exceeded');void browser.close();},180000);
...
async function capture(name){const r=await page.evaluate(()=>robotMovementRead());rows.push({name,...r});await page.screenshot({path:`${out}/${name}.png`,timeout:20000});console.log(name,r.state.phase,r.state.frameDt);return r;}
...
try{
 ...
 await page.locator('#course-list .course').first().click();
 ...
 await page.waitForFunction(()=>kyotoState.phase==='Result',null,{timeout:60000});await capture('result');
 await page.keyboard.press('KeyR');await page.waitForFunction(()=>kyotoState.phase==='Aim');await capture('result-retry');
 await page.keyboard.press('Escape');await page.locator('#leaderboard .score-row').first().click();
 ...
 await page.reload();await page.waitForFunction(()=>kyotoState.ready);await capture('fresh-again');
