...
const step=new Function('THREE','snapshot','azimuth','elevation','dt','manualCamera','phase','replaying',`const inFlight=true,toThree=v=>new THREE.Vector3(v.x,v.y,-v.z);${body};return {azimuth,elevation};`);
const run=(v,a=0,e=0,dt=1/60,manual=false,phase='Flight',replay=false)=>step(THREE,{velocity:v},a,e,dt,manual,phase,replay);
...
let rolling={azimuth:.8,elevation:-.4};for(let i=0;i<180;i++)rolling=run({x:.1,y:0,z:.1},rolling.azimuth,rolling.elevation,1/60,false,'Flight',true);
assert(rolling.elevation>.15);assert.equal(rolling.azimuth,.8);
assert.deepEqual(run({x:.1,y:0,z:.1},.8,-.4,.1,true,'Flight',true),{azimuth:.8,elevation:-.4});
