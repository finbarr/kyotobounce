...
export function arcadeFeedback(sound){
 ...
 function celebrate(mult,kind,label,audible=true){
  clearAnimations();const tier=Math.min(6,Math.max(0,Math.floor(Math.log2(Math.max(1,mult))))),goal=kind==='goal';
  spectacle.dataset.tier=tier;spectacle.dataset.kind=kind;spectacle.hidden=false;spectacleTime=goal?2.5:1.45;cooldown=goal?1.6:.85;
  ...
  if(audible)sound.cue(goal?'goal':'bank',{multiplier:mult});
 }
 ...
 return {
  reset(){lastUpdate=performance.now();pending=null;cooldown=0;spectacleTime=0;shownMultiplier=1;clearAnimations();spectacle.hidden=true;current=null;bankCount=0;tagged=false;display=10000;flash=0;trickTime=0;hud.hidden=true;hud.dataset.cue='';},
  accept(score,attempt){
   ...
   if(Number.isFinite(mult)&&mult>shownMultiplier&&((mult-shownMultiplier)>=Math.max(.25,shownMultiplier*.12)||newBank)){
    pending={mult,label:newBank?(score.lastBank||'CLEAN BANK'):'AIR TIME',banks:score.styleBanks};
   }
   if(score.goalVisited&&!tagged){tagged=true;pop('TARGET HIT!  25% BANKED', 'goal');pending=null;shownMultiplier=mult;celebrate(mult,'goal','TARGET HIT');}
  ...
  update(dt,phase,mode){
   ...
   if(pending&&!cooldown&&visible&&phase==='Flight'){const event=pending;pending=null;shownMultiplier=event.mult;celebrate(event.mult,'bank',event.label);}
