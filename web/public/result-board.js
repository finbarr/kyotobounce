// Rows are keyed by player, so improving an existing best moves that same row.
...
export function resultBoard({host,watch,sound}){
 ...
 panel.innerHTML='<div class="board-heading"><span class="eyebrow">LEVEL TOP 10 / ランキング</span><b id="ranking-announcement" role="status"></b></div><ol aria-label="Level high scores"></ol><p class="board-footnote">One best per player · select a score to watch</p>';
 ...
 function row(entry,rank,own){
  const li=document.createElement('li');li.dataset.guest=entry.guest;li.dataset.rank=rank;li.classList.toggle('own-score',own);
 ...
 function show(result){
  clear();const standings=result.standings;if(!standings)return;const {before,after,rank,previousRank,improved}=standings;
  const winner=rank&&rank<=10&&improved,own=after.find(e=>e.attempt===result.attempt)?.guest;
  ...
  later(()=>{
   ...
   later(()=>{
    ...
    const old=new Map([...list.children].map(li=>[li.dataset.guest,{li,top:li.getBoundingClientRect().top}]));
    const origin=list.getBoundingClientRect().top,removed=[...old].filter(([guest])=>!after.some(e=>e.guest===guest));
    ...
    for(const {entry,li}of newRows){
     const previous=old.get(entry.guest),top=li.getBoundingClientRect().top;
     if(entry.guest===own){li.classList.add('new-rank');animate(li,[{opacity:.2,transform:`translate(${previous?'0':'-70px'},${previous?previous.top-top:-30}px) scale(1.1)`},{opacity:1,transform:'translate(0,0) scale(1.04)',offset:.72},{opacity:1,transform:'translate(0,0) scale(1)'}],{duration:620,easing:'cubic-bezier(.18,.8,.23,1)'});}
    ...
    announcement.textContent=rank===1?'NEW NUMBER ONE!':rank<=3?`PODIUM! #${rank}`:previousRank?`YOUR BEST · #${rank}`:`TOP TEN! #${rank}`;
