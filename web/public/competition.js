...
export function competitionUI({scene,send,cancel,notice,getGuestId,getLiveTime,getPhase,resetView,overview,sound,archiveMode=false,onReplayClose,loadedLayout=()=>null}){
 ...
 const state={mode:'play',session:null,challenges:[],archived:[],archiveSelection:null,selected:null,board:[],draft:{start:null,goal:null,waypoints:[],scoring:'waypoint-v1'},selectedWaypoint:null,editId:null,placing:null,replay:null,replayTime:0,replayPlaying:false,lastResult:null,hint:null};
 ...
 function drawDisks(challenge){
  ...
  if(!challenge)return;
  const arcade=['combo-v4','combo-v5'].includes(challenge.scoring);
 ...
 function updateSession(){
  ...
  if(changed){feedback.reset();liveScore=null;scoreAttempt='';scoreEpoch++;state.board=[];renderBoard();$('result-card').hidden=true;}
  $('course-title').textContent=state.selected?.name||'PICK YOUR LINE.';$('challenge-detail').hidden=!state.selected;
  if(state.selected){const c=state.selected;$('challenge-description').textContent=c.scoring==='waypoint-v1'?`${c.waypoints?.length||0} optional waypoints · ${c.goal?'optional destination bonus':'no destination required'} · keep waypoint points on a destination miss`:`Start within ${c.start.radius.toFixed(2)} m · settle inside ${c.goal.radius.toFixed(2)} m${c.requiredSurface?' · required route contact':''}`;state.hint=state.selected.hint||null;$('use-hint').hidden=!state.hint;$('hint-note').textContent=state.hint?.note||'';$('edit-challenge').hidden=state.selected.creator!==getGuestId();}
  ...
  if(state.selected?.scoring==='waypoint-v1')$('score-guide').innerHTML='<summary>HOW TO SCORE</summary><p>Collect any waypoint once: 10,000, then 20,000, then 40,000… Banks and active time multiply the chain. The next award doubles after every new waypoint.</p><p>The gold destination is an optional landing bonus. Missing it keeps all waypoint points. Score locks at full rest; recall forfeits.</p>';
 ...
 async function renderArchiveBoard(c){
  state.archiveSelection=c;const host=$('archive-board');host.replaceChildren();const label=document.createElement('p');label.textContent=`${c.name} · revision ${c.revision} · ${c.archiveReason||'Read-only history'}`;host.append(label);
  if(c.creator===getGuestId()){const revise=document.createElement('button');revise.textContent='Revise for current station';revise.onclick=()=>openEditor(true,c);host.append(revise);}
  try{const response=await fetch(`/api/archive/catalog?challenge=${encodeURIComponent(c.id)}&revision=${c.revision}`,{cache:'no-store'});if(!response.ok)throw new Error('Archive board unavailable');const data=await response.json();if(state.archiveSelection!==c)return;
   if(!data.entries?.length){const empty=document.createElement('p');empty.textContent='No saved scores for this revision.';host.append(empty);}
   for(const entry of data.entries||[]){const same=c.layout===loadedLayout(),link=document.createElement(same?'button':'a');link.className='score-row';if(same)link.onclick=()=>send('replay',{attempt:entry.attempt});else link.href=replayNavigation(entry.attempt);link.textContent=`${entry.name} · ${Number(entry.score).toLocaleString()} · Watch matching-station replay`;host.append(link);}
  }catch(error){if(state.archiveSelection===c){const message=document.createElement('p');message.textContent=error.message;host.append(message);}}
 }
 function renderArchives(){const list=$('archive-list');list.replaceChildren();$('archive-courses').hidden=!state.archived.length;for(const c of state.archived){const button=document.createElement('button');button.className='course';button.dataset.challenge=c.id;button.dataset.revision=c.revision;button.textContent=`${c.name} · r${c.revision}`;button.onclick=()=>renderArchiveBoard(c);list.append(button);}}
 ...
 function openEditor(edit=false,archived=null){
  ...
  cancel();const current=archived||(edit?state.selected:null);
  state.editId=current?.id||null;state.draft=current?structuredClone({start:current.start,goal:current.goal,waypoints:current.waypoints||[],scoring:current.scoring==='waypoint-v1'?'waypoint-v1':'combo-v5'}):{start:null,goal:null,waypoints:[],scoring:'waypoint-v1'};state.selectedWaypoint=null;cancelPendingPlacement();
 ...
 function renderEditor(){
  const waypoint=state.draft.scoring==='waypoint-v1';$('waypoint-editor').hidden=!waypoint;$('destination-toggle').hidden=!waypoint;
 ...
 $('challenge-scoring').onchange=()=>{state.draft.scoring=$('challenge-scoring').value;if(state.draft.scoring!=='waypoint-v1'){state.draft.waypoints=[];state.selectedWaypoint=null;}state.placing=null;cancelPendingPlacement();renderEditor();drawDisks(state.draft);};
 ...
 $('close-replay').onclick=()=>{if(archiveMode){onReplayClose?.();return;}feedback.reset();state.mode='play';state.replay=null;state.replayPlaying=false;drawDisks(state.selected);presentation();resetView();};
 ...
 return {
  ...
  message(m){
   ...
   if(m.type==='catalog'){state.challenges=m.challenges;state.archived=m.archived||[];renderCatalog();renderArchives();}
   ...
   if(m.type==='replay'){if(!archiveMode&&loadedLayout()&&m.replay.layout!==loadedLayout()){location.assign(replayNavigation(m.replay.attempt));return;}loadReplay(m.replay);}
   if(m.type==='result'){
    ...
    const b=m.breakdown,waypoint=b?.version==='waypoint-v1',rank=waypoint?(['forfeit','route-missed'].includes(b.outcome)?b.outcome:b.destinationReached?'perfect':m.success?'chain':'miss'):b?.outcome||(m.success?'perfect':'miss');
    ...
    if(b)for(const [label,value]of waypoint?[['WAYPOINTS · '+b.waypointCount,b.waypointBase],['BANKS · '+b.styleBanks,'×'+b.bankMultiplier.toFixed(2)],['ACTIVE TIME · '+b.activeSeconds.toFixed(1)+'s','×'+b.timeMultiplier.toFixed(2)],['DESTINATION BONUS',b.destinationBonus]]:['combo-v4','combo-v5'].includes(b.version)?[['BASE',b.base],['BANKS · '+b.styleBanks,'×'+b.bankMultiplier.toFixed(2)],['ACTIVE TIME · '+b.activeSeconds.toFixed(1)+'s','×'+b.timeMultiplier.toFixed(2)],['LANDING',Math.round(b.landingMultiplier*100)+'%']]:[['ACCURACY',b.accuracy],['STYLE',b.style]]){
  ...
  update(dt){
   ...
   const course=state.mode==='replay'?state.replay?.challenge:state.selected;waypointHud.hidden=course?.scoring!=='waypoint-v1'||state.mode==='editor';
