...
export function competitionUI({onModeChange=()=>{},focusTarget=()=>{},scene,send,cancel,notice,getGuestId,getLiveTime,getPhase,resetView,overview,sound,sharedReplay=false,getLayout=()=>null}){
 ...
 function updateSession(){
  ...
  if(state.selected?.scoring==='waypoint-v3')$('score-guide').innerHTML='<summary>HOW TO SCORE</summary><p>10,000 base × (waypoint multiplier + bank bonus). Each unique waypoint doubles the waypoint multiplier: ×2, ×4, ×8… Each distinct bank adds +0.5× separately. Waypoints never double the bank bonus. Movement adds a flat 100 points per second after multipliers.</p><p>The gold destination is an optional landing bonus equal to your target score (10,000 × combo if no waypoints). Missing it keeps all waypoint points. Hit a target to bank the movement bonus. Score locks at full rest; recall forfeits.</p>';
