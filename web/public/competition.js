...
export function competitionUI({scene,send,cancel,notice,getGuestId,getLiveTime,getPhase,resetView,overview,sound}){
 ...
 function drawDisks(challenge){
  ...
  if(!challenge)return;
  const arcade=['combo-v6'].includes(challenge.scoring);
 ...
 function openEditor(edit=false){
  ...
  state.editId=current?.id||null;state.draft=current?structuredClone({start:current.start,goal:current.goal,waypoints:current.waypoints||[],scoring:current.scoring==='waypoint-v2'?'waypoint-v2':'combo-v6'}):{start:null,goal:null,waypoints:[],scoring:'waypoint-v2'};state.selectedWaypoint=null;cancelPendingPlacement();
 ...
 function renderEditor(){
  const waypoint=state.draft.scoring==='waypoint-v2';$('waypoint-editor').hidden=!waypoint;$('destination-toggle').hidden=!waypoint;
 ...
 $('challenge-scoring').onchange=()=>{state.draft.scoring=$('challenge-scoring').value;if(state.draft.scoring!=='waypoint-v2'){state.draft.waypoints=[];state.selectedWaypoint=null;}state.placing=null;cancelPendingPlacement();renderEditor();drawDisks(state.draft);};
 ...
 return {
  ...
  message(m){
   ...
   if(m.type==='result'){
    ...
    if(b)for(const [label,value]of waypoint?[['WAYPOINTS · '+b.waypointCount,b.waypointBase],['BANKS · '+b.styleBanks,'×'+b.bankMultiplier.toFixed(2)],['DESTINATION BONUS',b.destinationBonus]]:['combo-v6'].includes(b.version)?[['BASE',b.base],['BANKS · '+b.styleBanks,'×'+b.bankMultiplier.toFixed(2)],['LANDING',Math.round(b.landingMultiplier*100)+'%']]:[['ACCURACY',b.accuracy],['STYLE',b.style]]){
  ...
  update(dt){
   ...
   const course=state.mode==='replay'?state.replay?.challenge:state.selected;waypointHud.hidden=course?.scoring!=='waypoint-v2'||state.mode==='editor';
