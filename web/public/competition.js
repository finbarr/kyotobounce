...
export function competitionUI({onModeChange=()=>{},focusTarget=()=>{},scene,send,cancel,notice,getGuestId,getLiveTime,getPhase,resetView,overview,sound,sharedReplay=false,getLayout=()=>null}){
 ...
 <div id="replay-view" hidden><div class="panel-top"><span class="eyebrow">REPLAY / リプレイ</span><button id="close-replay">✕</button></div><h2 id="replay-title"></h2><div id="replay-score"></div><input id="replay-scrub" type="range" aria-label="Replay time" step="0.005" value="0"><div class="actions"><button id="replay-play">Pause</button><button id="replay-recenter">Recenter</button></div><div class="replay-share actions"><button id="share-replay">Copy replay link ↗</button><a id="play-replay-level" href="/">Play this level →</a></div><p id="replay-status" role="status">WASD to fly · Q/E down/up · Shift to boost. Click to look freely, or right-drag. Hold Space for 2× playback. Esc releases the mouse.</p></div>
 ...
 function updateSession(){
  ...
  if(state.selected){const c=state.selected;$('challenge-description').textContent=c.scoring==='waypoint-v3'?`${c.waypoints?.length||0} required waypoints · ${c.goal?(c.waypoints?.length?'optional destination bonus':'destination required'):'no destination required'} · collect every waypoint to pass and rank`:`Start within ${c.start.radius.toFixed(2)} m · settle inside ${c.goal.radius.toFixed(2)} m${c.requiredSurface?' · required route contact':''}`;if(c.campaign)$('challenge-description').textContent=c.campaign.brief+' '+$('challenge-description').textContent;state.hint=state.selected.hint||null;$('use-hint').hidden=!state.hint;$('hint-note').textContent=state.hint?.note||'';$('edit-challenge').hidden=state.selected.creator!==getGuestId();}
  if(state.mode==='play')drawDisks(state.selected);
  $('timing-hint').hidden=!state.selected?.hint?.period;
  $('score-guide').hidden=false;
  if(state.selected?.scoring==='waypoint-v3')$('score-guide').innerHTML='<summary>HOW TO SCORE</summary><p>10,000 base × (waypoint multiplier + bank bonus). Each unique waypoint doubles the waypoint multiplier: ×2, ×4, ×8… Each distinct bank adds +0.5× separately. Waypoints never double the bank bonus. Movement adds a flat 100 points per second after multipliers.</p><p>The gold destination is an optional landing bonus equal to your target score (10,000 × combo if no waypoints). Collect every waypoint to pass and enter the leaderboard, with or without the destination. Missing a waypoint leaves the shot unranked. With no waypoints, land in the destination to bank the score. Score locks at full rest; recall forfeits.</p>';
 ...
 async function openReplay(attempt){
  ...
  try{const replay=await fetchReplay(attempt);if(request!==replayRequest)return;if(state.session?.busy&&!['Aim','Result'].includes(getPhase()))throw new Error('Finish or recall your shot before watching a replay.');if(getLayout()&&replay.layout!==getLayout())throw new Error('This replay uses a station version that is no longer available.');loadReplay(replay);history.replaceState(null,'',replayURL(attempt));notice('Replay ready · WASD to fly · Right-drag to look · Hold Space for 2×');}
  catch(error){if(request!==replayRequest)return;notice(error.message,15000);if(sharedReplay){$('replay-status').textContent=error.message;$('replay-title').textContent='REPLAY UNAVAILABLE';$('play-replay-level').textContent='Play Kyoto Bounce →';for(const id of ['replay-play','replay-scrub','replay-recenter','share-replay'])$(id).disabled=true;$('replay-view').hidden=false;$('course-view').hidden=true;}}
 }
 function loadReplay(replay){
  ...
  for(const id of ['replay-play','replay-scrub','replay-recenter','share-replay'])$(id).disabled=false;
 ...
 if(sharedReplay){$('replay-title').textContent='LOADING REPLAY…';for(const id of ['replay-play','replay-scrub','replay-recenter','share-replay'])$(id).disabled=true;}
 $('close-replay').onclick=()=>{replayRequest++;if(sharedReplay){location.href=$('play-replay-level').href;return;}history.replaceState(null,'','/');document.title='Kyoto Bounce — Station Arcade';feedback.reset();setMode('play');state.replay=null;state.replayPlaying=false;drawDisks(state.selected);presentation();resetView();};
 $('replay-play').onclick=()=>{cancel();if(state.replayTime>=state.replay.duration)state.replayTime=replayStart();state.replayPlaying=!state.replayPlaying;$('replay-play').textContent=state.replayPlaying?'Pause':'Play';};
 $('replay-scrub').oninput=()=>{cancel();scoreEpoch++;feedback.reset();state.replayTime=Number($('replay-scrub').value);state.replayPlaying=false;$('replay-play').textContent='Play';};$('replay-recenter').onclick=()=>resetView();
 ...
 return {
  ...
  message(m){
   ...
   if(m.type==='saved-challenge'){setMode('play');presentation();send('select-challenge',{challengeId:m.challenge.id,revision:m.challenge.revision});notice('Challenge saved. Set the first high score.');}
   if(m.type==='selected'){replayRequest++;$('result-card').hidden=true;resetView('level');if(m.challenge&&state.mode==='play')overview(state.selected||m.challenge);}
   ...
   if(m.type==='result'){
    ...
    const b=m.breakdown,waypoint=b?.version==='waypoint-v3',rank=waypoint?(['forfeit','route-missed','incomplete'].includes(b.outcome)?b.outcome:b.destinationReached?'perfect':m.success?'chain':'miss'):b?.outcome||(m.success?'perfect':'miss');
    ...
    $('result-rank').textContent=allClear?'ALL WAYPOINTS!':({chain:'WAYPOINTS BANKED',perfect:'PERFECT LANDING',tagged:'TAGGED IT!',near:'SO CLOSE',miss:'FIND YOUR LINE',incomplete:m.challenge?.waypoints?.length?'WAYPOINTS MISSED':'DESTINATION MISSED',forfeit:'SHOT RECALLED','route-missed':'ROUTE MISSED'})[rank];
    $('result-title').textContent=`${m.score.toLocaleString()} PTS`;
    $('result-breakdown').textContent=waypoint?(b.outcome==='forfeit'?'Recalling a live shot forfeits its points.':b.outcome==='route-missed'?'The required route contact was missed; this shot earns no points.':b.outcome==='incomplete'?`${b.waypointCount} / ${m.challenge?.waypoints?.length||0} waypoints collected. ${m.challenge?.waypoints?.length?'Collect every waypoint to pass and rank.':'Land in the destination to pass and rank.'} This shot is unranked.`:`${b.waypointCount} / ${m.challenge?.waypoints?.length||0} waypoints collected. ${b.destinationReached?'Destination bonus earned.':m.challenge?.goal?'Destination bonus missed; full waypoint score banked.':'Full waypoint score banked.'}`):b?b.outcome==='perfect'?'Settled inside the target.':b.outcome==='tagged'?'Entered the target, then bounced out.':b.outcome==='forfeit'?'Recalling a live shot forfeits its points.':b.outcome==='route-missed'?'This stage requires an escalator contact.':`${b.distance.toFixed(2)} m from the target when the ball stopped.`:m.success?`1000 target + ${m.surfaces} surfaces × 100`:m.challenge?'Try another angle.':'Choose a stage to save your score.';
    ...
    $('result-tip').textContent=waypoint?(m.challenge?.waypoints?.length?'Collect EVERY waypoint in any order to pass and rank. R · TRY AGAIN':'Land in the destination to pass and rank. R · TRY AGAIN'):b?.outcome==='miss'?`Finish within ${b.proximityRange.toFixed(1)} m to earn proximity points.`:'R · TRY AGAIN     ESC · REPLAY & MENUS';
