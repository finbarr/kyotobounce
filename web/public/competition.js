...
export function competitionUI({scene,send,cancel,notice,getGuestId,getLiveTime,getPhase,resetView,overview,sound}){
 ...
 function drawDisks(challenge){
  ...
  if(!challenge)return;
  const arcade=challenge.scoring==='combo-v4'||state.mode==='editor';
 ...
 $('next-challenge').onclick=()=>{const next=state.challenges.find(c=>c.order===(state.selected?.order??-2)+1);if(next)send('select-challenge',{challengeId:next.id,revision:next.revision});};
 return {
  state,
  ...
  message(m){
   ...
   if(m.type==='result'){
    ...
    if(b)for(const [label,value]of b.version==='combo-v4'?[['BASE',b.base],['BANKS · '+b.styleBanks,'×'+b.bankMultiplier.toFixed(2)],['ACTIVE TIME · '+b.activeSeconds.toFixed(1)+'s','×'+b.timeMultiplier.toFixed(2)],['LANDING',Math.round(b.landingMultiplier*100)+'%']]:[['ACCURACY',b.accuracy],['STYLE',b.style]]){
