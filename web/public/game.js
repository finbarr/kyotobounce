...
let ui,briefing,names,mobile,startup,explicitMouseRelease=false;
const levelMenuOpen=()=>mobile?.active?mobile.menu==='levels':!!ui?.levelsOpen;
function openLevelMenu(){
 if(!ui||ui.state.mode!=='play'||names?.active||startup?.active)return;
 if(['Release','Flight'].includes(shotTimeline?.phase||snapshot?.phase)){notice('Finish your shot, or press R to recall it, then L for levels.');return;}
 if(briefing.active){briefing.start(false);return;}
 if(mobile.active){cancel();ui.dismissResult();mobile.openLevels();ui.focusLevel();}else ui.openLevels();
}
function closeLevelMenu(){mobile?.close();ui?.closeLevels();canvas.focus();captureMouse();}
...
function setFastForward(held){
 fastForwardHeld=held;
 if(ui?.state.mode==='replay'){sound.setPlaybackRate(ui.playbackRate(held));return;}
 const rate=held&&connectionStatus==='connected'&&['play','design'].includes(ui?.state.mode)&&shotTimeline?.phase==='Flight'&&shotPlayback.active?2:1;
 if(rate===livePlaybackRate)return;
 livePlaybackRate=rate;shotPlayback.setRate(rate,performance.now());sound.setPlaybackRate(rate);
 const attempt=shotPlayback.shot?.attempt;if(attempt)send('playback-rate',{attempt,rate});
}
speedIndicator.onclick=()=>{sound.unlock();setFastForward(!fastForwardHeld);canvas.focus();};
...
const connection=sharedReplayId?null:gameConnection({
  ...
  hello:()=>({protocol:'shot-stream-v4',token:localStorage.getItem('kyoto-guest'),sessionId:guestId,lastResultAttempt}),
...
document.addEventListener('mousemove',event=>{
  ...
  if(['play','design'].includes(ui.state.mode)){
...
window.addEventListener('keydown',e=>{
  if(e.defaultPrevented||names?.active||startup?.active||e.isComposing)return;
  const editing=e.target.closest('input,textarea,select,[contenteditable="true"]'),modified=e.metaKey||e.ctrlKey||e.altKey;
  if(!editing&&!modified&&e.code==='KeyL'&&ui.state.mode==='play'){e.preventDefault();if(!e.repeat){if(levelMenuOpen())closeLevelMenu();else openLevelMenu();}return;}
  if(briefing?.active){if(e.code==='Enter'&&e.target.closest('button')&&e.target.id!=='briefing-play')return;if(['Space','Enter'].includes(e.code)&&!modified){e.preventDefault();if(!e.repeat)briefing.start();}if(e.code==='Escape'){e.preventDefault();briefing.start(false);}return;}
  ...
  if(e.code==='Escape'&&ui.state.mode==='play'){e.preventDefault();openLevelMenu();return;}
  if(['INPUT','TEXTAREA','SELECT','BUTTON','SUMMARY'].includes(e.target.tagName))return;
  if(e.code==='KeyF'&&!e.repeat){e.preventDefault();if(ui.state.mode==='replay'&&fly.active)$('replay-recenter').click();else setFly(!fly.active,ui.state.mode!=='replay');return;}
  if(ui.state.mode==='replay'&&e.code==='Space'){e.preventDefault();if(!e.repeat){sound.unlock();setFastForward(true);}return;}
  if(fly.active){if(['Space','KeyW','KeyA','KeyS','KeyD','KeyQ','KeyE','ControlLeft','ControlRight'].includes(e.code))e.preventDefault();if(e.code==='Escape'){cancel();return;}keys.add(e.code);return;}
  if(ui.state.mode==='replay'){if(e.code==='Escape'){cancel();return;}if(e.code.startsWith('Arrow')){e.preventDefault();keys.add(e.code);}return;}
  if(['Space','ArrowUp','ArrowDown','ArrowLeft','ArrowRight'].includes(e.code))e.preventDefault();
  if(e.code==='Space'&&['play','design'].includes(ui.state.mode)&&shotTimeline?.phase==='Flight'){if(!e.repeat)setFastForward(true);return;}
  keys.add(e.code);if(e.repeat)return;
  if(e.code==='KeyH'&&ui.state.hint&&ui.state.mode==='play')$('use-hint').click();
  if(e.code==='Space'){captureMouse();startCharge();}if(e.code==='Escape')cancel();
...
function sendInput(){if(names?.active||briefing?.blocked||!workerReady||!loaded||ui.state.mode==='replay'||shotPlayback.active)return;send('input',{x:fly.active||ui.levelsOpen||mobile?.menu||ui.state.mode==='editor'||flightControls()?0:THREE.MathUtils.clamp(Number(keys.has('KeyD'))-Number(keys.has('KeyA'))+(mobile?.movement.x||0),-1,1),z:fly.active||ui.levelsOpen||mobile?.menu||ui.state.mode==='editor'||flightControls()?0:THREE.MathUtils.clamp(Number(keys.has('KeyW'))-Number(keys.has('KeyS'))+(mobile?.movement.z||0),-1,1),fast:keys.has('ShiftLeft')||keys.has('ShiftRight')||!!mobile?.movement.fast,yaw,pitch,top,kick});}
...
if(sharedReplayId)$('course-list').after(characterPicker.element);else $('loading-character').append(characterPicker.element);
...
function animate(now){
  ...
  if(replaying?!ui.state.replayPlaying:shotTimeline?.phase!=='Flight'||!['play','design'].includes(ui.state.mode))setFastForward(false);
  ui.update(dt,replaying?ui.playbackRate(fastForwardHeld):livePlaybackRate);
  if(replaying&&!ui.state.replayPlaying)setFastForward(false);
  const presentationRate=replaying?ui.playbackRate(fastForwardHeld):livePlaybackRate;
  sound.setPlaybackRate(presentationRate);
  ...
  if(snapshot){
    ...
    guide.visible=!fly.active&&!inFlight&&!!p&&['play','design'].includes(ui.state.mode);
  }
  ...
  heat.update(scorePresentation,dt*presentationRate,phase,matchMedia('(prefers-reduced-motion: reduce)').matches,rawDt*1000);
  if(ui.state.mode==='design'){ball.material.color.setHex(0x5fffe3);ball.material.emissive.setHex(0x167b72);ball.material.emissiveIntensity=.3;trail.material.color.setHex(0x5fffe3);trail.material.opacity=.7;}
  ...
  if(inFlight&&!manualCamera&&phase==='Flight'&&snapshot?.velocity){
    ...
    // A slow rolling replay needs a view above the floor, not the downward
    // camera offset left over from its last upward bounce.
    if(replaying&&Math.abs(velocity.y)<.2&&horizontal<3)elevation=approach(elevation,.16,.8);
    else if(velocity.length()>.75)elevation=approach(elevation,THREE.MathUtils.clamp(-Math.atan2(velocity.y,horizontal),-.95,.95),.8);
  }
  camera=inFlight?ballCamera:aimCamera;
  const resultAvatar=avatars.get(replaying?p?.id:guestId),showRobotResult=inFlight&&phase==='Result'&&avatarWantsResultView(resultAvatar);
  ...
  scoutTargets.update(camera,ui.state.mode==='editor'?ui.state.draft:ui.state.selected,fly.active&&!replaying);
  flyControl.hidden=briefing.blocked||replaying||['Charging','Release','Flight'].includes(phase);
  flyControl.dataset.speed=String(fly.speed);
  ...
  const reticle=!fly.active&&p&&['play','design'].includes(ui.state.mode)&&!inFlight&&!flightPending&&phase!=='Release'?projectAimReticle(release,aim,camera,station):null;
  ...
  if(!['play','design'].includes(ui.state.mode)){$('throw-panel').hidden=true;$('flight-panel').hidden=true;$('reticle').style.display='none';}
  ...
  $('power-control').hidden=fly.active||!snapshot||inFlight||!['play','design'].includes(ui.state.mode)||briefing.blocked;
  sound.motion(snapshot?Math.hypot(snapshot.velocity.x,snapshot.velocity.y,snapshot.velocity.z):0,snapshot?.diagnostics?.supported,phase==='Flight'&&presentationRate>0&&['play','design','replay'].includes(ui.state.mode));
