...
export function challengeBriefing({renderer,onStart,onOpen,isReady}){
 ...
 host.innerHTML=`<div class="briefing-heading"><span class="eyebrow">STAGE PREVIEW / コース</span><h2 id="briefing-name"></h2><p>Find your line through Kyoto Station.</p><button id="briefing-back">← STAGE SELECT <kbd>ESC</kbd></button></div><div class="map-label start-label" id="map-start"><b>01</b><span>THROW FROM HERE<small id="map-start-radius"></small></span></div><div class="map-label goal-label" id="map-goal"><b>◎</b><span><strong id="map-goal-name">LAND HERE</strong><small id="map-goal-radius"></small></span></div><div class="briefing-bottom"><div class="briefing-facts"><span id="briefing-distance"></span><p id="briefing-rule"></p><p class="map-key"><i></i> Stay inside the blue start circle.<br><i></i> Settle the ball inside the gold target.</p></div><div class="legacy-points">LEGACY RULES<br><b>1,000</b> for settling inside<br>+100 per distinct surface</div><div class="briefing-points"><span>BASE SCORE <b>10,000 × COMBO</b></span><span>EACH NEW BANK <b>×1.75</b></span><span>BULLSEYE LANDING <b>KEEP 100%</b></span><small>Build banks × time. Outer rings keep 75 / 50 / 25%.</small></div><button id="briefing-play" class="primary">LET’S BOUNCE <span>▶</span><small>SPACE TO START</small></button></div><div class="cutaway-note">STATION CUTAWAY · START & TARGET TO SCALE</div>`;
 ...
 function open(c){
  ...
  $('map-goal').hidden=!c.goal;$('map-goal-name').textContent=c.scoring==='waypoint-v1'?'OPTIONAL BONUS':'LAND HERE';$('map-goal-radius').textContent=c.goal?`${c.goal.radius.toFixed(2)} m ${c.scoring==='waypoint-v1'?'bonus':'target'} radius`:'';
  ...
  $('briefing-distance').textContent=`${length.toFixed(1)} m TO ${c.goal?'DESTINATION':'FARTHEST WAYPOINT'}${Math.abs(rise)>.1?` / ${rise>0?'+':''}${rise.toFixed(1)} m ELEVATION`:''}`;
 ...
 return {open,start,get target(){return target;},get active(){return active;},get blocked(){return active||transition>0;},get challenge(){return challenge;},
  update(camera,dt){
   if(active){
    ...
    const classic=['combo-v4','combo-v5'].includes(challenge.scoring);
    ...
    clip.constant=Math.max(...routeTargets(challenge).map(d=>d.center.y+d.radius))+2.6;renderer.clippingPlanes=[clip];
