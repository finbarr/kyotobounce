...
export function challengeBriefing({renderer,onStart,onOpen,isReady}){
 ...
 host.innerHTML=`<svg id="briefing-route" aria-hidden="true"></svg><div class="briefing-heading"><span class="eyebrow">STAGE PREVIEW / コース</span><h2 id="briefing-name"></h2><p id="briefing-description">Find your line through Kyoto Station.</p><button id="briefing-back">← STAGE SELECT <kbd>ESC</kbd></button></div><div class="map-label start-label" id="map-start"><b aria-hidden="true">⚑</b><span>SUGGESTED START<small id="map-start-radius"></small></span></div><div class="map-label goal-label" id="map-goal"><b>◎</b><span><strong id="map-goal-name">LAND HERE</strong><small id="map-goal-radius"></small></span></div><div class="briefing-bottom"><div class="briefing-facts"><span id="briefing-distance"></span><p id="briefing-rule"></p><p class="map-key"><i></i> Checkered pad = suggested start. Walk and throw anywhere.<br><i></i> Settle the ball inside the gold target.</p></div><div class="briefing-points"><span>BASE SCORE <b>10,000 × COMBO</b></span><span>EACH NEW BANK <b>+0.5×</b></span><span>BULLSEYE LANDING <b>KEEP 100%</b></span><small>Link distinct banks. Outer rings keep 75 / 50 / 25%. Add 100 PTS / SEC while moving.</small></div><button id="briefing-play" class="primary">LET’S BOUNCE <span>▶</span><small>SPACE TO START</small></button></div><div class="cutaway-note">STATION CUTAWAY · TARGET GUIDE</div>`;
 ...
 function open(c){
  ...
  $('map-start-radius').textContent='Walk anywhere · Choose your launch spot';
  ...
  $('briefing-rule').textContent=c.requiredSurface?'WALK & THROW ANYWHERE · Touch the moving escalator to clear the course.':'WALK & THROW ANYWHERE.';
  const waypoint=c.scoring==='waypoint-v3';host.classList.toggle('waypoint-briefing',waypoint);
  if(waypoint){$('briefing-rule').textContent=`WALK & THROW ANYWHERE · ${c.waypoints?.length||0} REQUIRED WAYPOINTS · ANY ORDER · ONCE PER SHOT${c.requiredSurface?' · REQUIRED ROUTE CONTACT':''}`;host.querySelector('.map-key').innerHTML='<i></i> Checkered pad = suggested start. Walk and throw anywhere.<br><i></i> Every in-bounds shot can rank. Collect every purple patch to clear.<br><i></i> Gold is a landing bonus; required to clear destination-only courses.';host.querySelector('.briefing-points').innerHTML='<span>FIRST WAYPOINT <b>×2 TARGETS</b></span><span>WAYPOINT COMPONENT <b>DOUBLES</b></span><span>DESTINATION <b>OPTIONAL BONUS</b></span><small>Each new bank adds +0.5× separately. Movement adds 100 PTS / SEC separately. Full rest locks the score.</small>';}
  else{host.querySelector('.map-key').innerHTML='<i></i> Checkered pad = suggested start. Walk and throw anywhere.<br><i></i> Settle inside the gold target.';host.querySelector('.briefing-points').innerHTML='<span>BASE SCORE <b>10,000 × COMBO</b></span><span>EACH NEW BANK <b>+0.5×</b></span><span>BULLSEYE LANDING <b>KEEP 100%</b></span><small>Link distinct banks. Outer rings keep 75 / 50 / 25%. Add 100 PTS / SEC while moving.</small>';}
