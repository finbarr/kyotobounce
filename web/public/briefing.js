...
export function challengeBriefing({renderer,onStart,onOpen,isReady}){
 ...
 function open(c){
  ...
  $('map-goal').hidden=!c.goal;$('map-goal-name').textContent=c.scoring==='waypoint-v2'?'OPTIONAL BONUS':'LAND HERE';$('map-goal-radius').textContent=c.goal?`${c.goal.radius.toFixed(2)} m ${c.scoring==='waypoint-v2'?'bonus':'target'} radius`:'';
 ...
 return {open,start,get target(){return target;},get active(){return active;},get blocked(){return active||transition>0;},get challenge(){return challenge;},
  update(camera,dt){
   if(active){
    ...
    const classic=['combo-v6'].includes(challenge.scoring);
