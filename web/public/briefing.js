...
export function challengeBriefing({renderer,onStart,onOpen,isReady}){
 ...
 function open(c){
  ...
  const waypoint=c.scoring==='waypoint-v1';host.classList.toggle('waypoint-briefing',waypoint);
 ...
 return {open,start,get target(){return target;},get active(){return active;},get blocked(){return active||transition>0;},get challenge(){return challenge;},
  update(camera,dt){
   if(active){
    ...
    const classic=['combo-v5'].includes(challenge.scoring);
    ...
    clip.constant=Math.max(...routeTargets(challenge).map(d=>d.center.y))+1.8;renderer.clippingPlanes=[clip];
    if(guideLine){
