...
export class Competition {
 ...
 async add(guest:Guest,id:string){
  ...
  if(selected&&(selected.throwModel!==THROW_MODEL||selected.scoring!==SCORING_VERSION))selected=this.store.challenge(selected.id);
 ...
 state(message:any){
  ...
  if(m){
   if(m.combo&&message.attempt===m.attempt&&['Flight','Result'].includes(message.phase)){
    ...
    if(!last||message.flightTime-last.t>=.1||last.score.styleBanks!==message.liveScore.styleBanks||last.score.goalVisited!==message.liveScore.goalVisited)m.scoreFrames?.push({t:message.flightTime,score:message.liveScore});
   }
 ...
 result(result:NativeResult){
  ...
  if(c){
   ...
   if(c.scoring===SCORING_VERSION){result.breakdown=scoreAttempt(result);result.score=result.breakdown.total;result.scoreFrames=[...(scoreFrames||[]),{t:result.duration,score:result.breakdown}];}
 ...
 async command(member:Member,m:any):Promise<unknown>{
  ...
  if(m.type==='place'){
   if(!['start','goal'].includes(m.slot))throw new Error('Choose start or goal');validateVector(m.origin);validateVector(m.direction);
  ...
  if(m.type==='save-challenge'){
   ...
   const start=disk(m.start),goal=disk(m.goal);await this.worker.request({type:'validate',id,start,goal});
   ...
   const challenge:Challenge={id:old?.id||randomUUID(),revision:(old?.revision||0)+1,name:m.name.trim(),creator:member.guest.id,start,goal,layout:this.layout,physics:this.physics,throwModel:THROW_MODEL,scoring:SCORING_VERSION};
