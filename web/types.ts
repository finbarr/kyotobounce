...
export const WAYPOINT_SCORING='waypoint-v3';
export const ACTIVE_SCORING=['combo-v7',WAYPOINT_SCORING];
...
export { THROW_MODEL };
export const SCORING_VERSION='combo-v7';
...
export type CourseProgress={challengeId:string;revision:number;completed:boolean;bestScore:number;rank:number|null};
export type Standings={before:LeaderboardEntry[];after:LeaderboardEntry[];rank:number|null};
export type DesignCapture={start:Disk;goal:Disk|null;note?:string;waypointTimes:number[];waypoints:Waypoint[];path:Vector[];duration:number;error:string};
export type NativeResult = {type:'result';design?:DesignCapture|null;attempt:string;id:string;reason:string;layout:string;physics:string;profile:string;challenge:Challenge|null;success:boolean;records?:{personalBest:boolean;courseBest:boolean};playerName?:string;character?:'ori'|'koma'|'don';standings?:Standings;destinationReached?:boolean;waypointHits?:WaypointHit[];score:number;surfaces:number;impacts:number;duration:number;releaseTime:number;chargeTime:number;thrower:unknown;launchPosition:Vector;velocity:Vector;spin:Vector;poses:{t:number;p:Vector;q:{x:number;y:number;z:number;w:number}}[];contacts:{surface:string;label:string;point:Vector;speed:number;time:number;qualifying:boolean}[];scoreFrames?:{t:number;score:import('./scoring.ts').ScoreBreakdown}[];breakdown?:import('./scoring.ts').ScoreBreakdown};
