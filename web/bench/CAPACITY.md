...
## Coordinator Mac integration evidence

The coordinator reports full-resolution Chrome/Metal testing on a real Mac GPU with median RAF about 16.7ms. In the controlled 120ms ordered-packet-delay wall stage, baseline had 33 backward render-clock steps and 396 stalls; the fix had zero backward steps and 232 stalls. In steady walking, baseline had 15 reversals and 128 stalls versus zero of each with the fix. Both preserved wall Z=-27.10000038. Coordinator receipts: `.local/fleet/evidence/mac-timing-comparison.json` in the root integration workspace; these receipts are not present in this benchmark checkout, so these are coordinator-reported results, not a rerun here.

This confirms the backward-time defect is fixed on the Mac GPU. It does **not** establish that all game jitter is solved: the large delay still starves the buffer, and occasional forward catch-up jumps remain. The isolated native/service measurements above supply a separate possible server contribution: approximately 24–26Hz publication and occasional gaps up to 220.7ms occurred without sustained resource saturation or observed transport backlog. Such gaps can deplete the renderer's available samples, but these separate runs do not attribute any particular Mac stall to the server. Internal serial-worker queue depth remains unmeasured; stable native pace and low sampled backlog do not rule out short publication or queue delays. Native publication/serialization instrumentation remains the proposed next diagnostic, with no render, native or infrastructure change made here.

Coordinator camera and compact-power browser acceptance passed. Core Space-next/repeat/keyup acceptance passed; the intended failed-result case unexpectedly completed Return Ticket, so the coordinator is rerunning that fixture with deliberately diverted aim. This is not currently evidence of an input defect. Gameplay/UI ownership stays with the coordinator and waypoint lane.
